...
def get_equivalent_contacts():
    ...
    for pdb in pdb_codes:
        ...
        for pdb, v in contacts_dict.items():
            ...
            equiv_contacts[str(species[pdb])+ str(germ_line[pdb]['light'])+": "+str(pdb)]=eq[pdb]
    return equiv_contacts



with open('equiv_contacts_list_all_contacts.json', 'w') as ctr: #save the contact residues in a json file
